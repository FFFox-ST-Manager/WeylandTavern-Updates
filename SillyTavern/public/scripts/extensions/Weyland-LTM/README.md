...
Full-JS replacement for the STscript LTM pipeline (Sleep / LTMPrompt / LTMDisabler / MemorySaver). v1.5.2.
...
**Suggestion nudge.** The old floating "Time for an LTM?" chip is gone (too subtle on desktop, too big on mobile). Instead the brain QR button (`fa-brain` inside `.qr--button`) gets `.wlm-brain-due` — orange + pulse — via `applyBrainDue()`, re-asserted by a MutationObserver on `#qr--bar` because QR re-renders wipe the class. Generating/ready/failed still use the chip (transient + actionable).

**Auto-LTM (v1.5.1).** `settings.autoLtmMode`: `'off'` (default) | `'semi'` | `'full'`.
- **Off** — unchanged manual flow; brain glows when `getEffectiveGoal()` is reached, user clicks "+ New LTM" themselves.
- **Semi** — `maybeAutoTrigger()` (called from every `updateChip()` pass) fires `runAutoJob()` the instant the cap is hit, generating a draft in the background with `job.autoTriggered = true`. It is NOT saved automatically — the brain keeps glowing (instead of the normal "draft ready" chip) until the user opens the panel and hits Save themselves. `onRerollClicked` already reuses `job.range` unchanged for a job-kind draft, so rerolling this draft — even much later — always regenerates from the original span, never a grown one.
- **Full** — same auto-trigger, but `runAutoJob()` calls `autoSaveJob()` the moment the draft passes `validateLTMShape()`. The brain never glows in this mode; the only feedback is the existing transient "🧠 LTM generating…" chip.
...
Auto-trigger only evaluates the **currently active** chat (same scope limitation the brain-due check always had) — it runs off `SillyTavern.getContext().chat`, so a chat you're not currently viewing won't auto-generate/cascade in the background.
